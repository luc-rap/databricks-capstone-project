...
def add_interview_note(
    ...
    try:
        result = lakebase_adapter.add_interview_note(
            user_email=user_email,
            job_id=job_id,
        ...
        logger.exception(f"Failed to add interview note")
        return {
            "status": "error",
...
@mcp.tool
def get_stale_applications(days_threshold: int = 14) -> dict:
    """
    ...
    Returns:
        ...
        - count: Number of stale applications
        - applications: List of stale application dicts with job details
    """
    user_email = _get_end_user_email()
    logger.info(f"get_stale_applications called by {user_email}, threshold={days_threshold} days")
    
    try:
        applications = lakebase_adapter.get_stale_applications(
            user_email=user_email,
        ...
        logger.exception(f"Failed to get stale applications")
        return {
            "status": "error",
            "message": f"Failed to get stale applications: {str(e)}",
            "applications": []
        }
